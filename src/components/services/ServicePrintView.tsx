...
export function generateServicePDF(data: PrintData): void {
  ...
  // Header: church name + service title
  doc.setFontSize(22);
  ...
  const statusStr = `Status: ${service.status.charAt(0).toUpperCase() + service.status.slice(1)}`;
  doc.text(statusStr, margin, 51);
  ...
  if (items.length === 0) {
    ...
    doc.autoTable({
      ...
      didParseCell: (data: Record<string, unknown>) => {
        // Gray background for segments
        if (data.section === 'body' && (data as any).row) {
          const rowIndex = (data as any).row.index;
          if (rowIndex !== undefined && items[rowIndex]?.type === 'segment') {
            (data as any).cell.styles.fillColor = [245, 245, 250];
          }
        }
      },
    });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const footerText = `Generated by WorshipCenter · ${new Date().toLocaleDateString()} · Page ${i} of ${pageCount}`;
